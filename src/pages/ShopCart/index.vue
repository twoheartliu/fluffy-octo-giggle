      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="item in cartInfoList" :key="item.id">
          ...
          <li class="cart-list-con5">
            <a class="mins" @click="skuNumHandler('minus', -1, item)">-</a>
            <input
              autocomplete="off"
              type="text"
              minnum="1"
              class="itxt"
              :value="item.skuNum"
              @change="skuNumHandler('change', $event.target.value * 1, item)"
            />
            <a class="plus" @click="skuNumHandler('plus', 1, item)">+</a>
          </li>
...
export default {
  ...
  methods: {
    ...
    skuNumHandler: throttle(async function (type, disNum, item) {
      switch (type) {
        case 'minus':
